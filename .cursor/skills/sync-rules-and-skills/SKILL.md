...
3. **单向收敛、避免双写**  
   ...
   - **例外**：rule **as-api-dynamic-query-authority** 为 **P0、alwaysApply** 的「API 事实层级」硬约束，正文刻意独立保留，**不要**为压缩而并入其它 Rule；细节仍以技能 **as-api-dynamic-query** 为准。
4. **不改变语义，不丢信息**  
...
- **`as-api-dynamic-query`**：UE 内 ASApiQuery 动态查询（**查 AS API 的首选**；能开 UE 时禁止用 grep Kit AS_API 代替）。  
- **`angelscript-api-query`**：仅当**无法使用 ASApiQuery** 时的离线查询（Kit AS_API → Hazelight）与可选写回。  
  - Rule 中 API 顺序应指向 `write-angelscript` / `as-api-dynamic-query`，勿在 Rule 里展开长流程。
...
- **`retro-automation-workflow`**：复盘落盘时新增易错点，**优先**写入 **`content/knowledge/05-gotchas.md`** 与对应 **Skill**（如 **write-angelscript**），**不要**把长段落写进 Rule；与上文「Rule 只做触发」一致。
