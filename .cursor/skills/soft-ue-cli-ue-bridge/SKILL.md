...
**蓝图“事实”的权威来源**（须在 `py -3 -m soft_ue_cli check-setup` **成功**之后）：**单资产 / 全图对齐**仍用 `query-blueprint` / `query-blueprint-graph` / `query-asset` / `find-references` 等。**固定 scope 内批量读图结构、语义（`semantic`/`pin_type`/`edge_kind`）**时，优先 **`bp-index-refresh`**（`L0L1L2` + `l2_projection` + `l2_semantic_level`）→ **`bp-index-l2-list`** → **`bp-index-chunk-get`**，全图 Query 作回落；见 ManteumTower `Docs/Blueprint-Indexer-L2-Semantic-Code-Equivalence.md` §6。Kit 侧**入口分流表**：**[content/knowledge/07-blueprint-query-workflow.md](../../content/knowledge/07-blueprint-query-workflow.md)** §0 / §1.1.2。
