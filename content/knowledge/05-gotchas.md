...
| `Build.bat` / UBT **退出码 6**（但看不出原因） | 退出码本身不够信息量；常见两类：**Live Coding mutex** 或 **真实 C++ 编译错误** | 以 `Engine/Engine/Engine/Programs/UnrealBuildTool/Log.txt` **尾部**为准先分流：若含 `Unable to build while Live Coding is active` → 关 UE 或 Ctrl+Alt+F11；若是 `error Cxxxx`/链接错误 → 按报错修代码再编。**删除 `Log-backup-*.txt` 只适用于日志 I/O/磁盘空间/权限/占用等问题**，不能解决 Live Coding 或编译错误 |
| soft-ue-cli 的 `bp-index-chunk-get --chunk-id <id>` 在 Windows 上行为异常/报错 | `chunk_id` 内含 `|`，当调用链经过 **cmd.exe**（如 `py.cmd` stub、某些测试 harness）时，`|` 会被当作管道符拆分参数 | 对 `chunk_id` **加引号**（确保作为一个 argv 传入），例如：`--chunk-id "/Game/BP_A.BP_A|uber_graph|EventGraph"`；在 PowerShell 中建议用变量承载并直接传入，避免转义混乱；脚本/测试里需覆盖该场景 |
...
| AS 里写 `Debug::DrawDebugArrow` 等报 **Namespace 'Debug' doesn't exist** | UE/AS 绑定里调试绘制通常在 **`System::`** 下（与 `UKismetSystemLibrary` 风格一致），不存在独立 `Debug::` 命名空间 | **能开 UE**：**as-api-dynamic-query**（`filter`=`DrawDebug` 或 `System::`）；**离线**：`API_Docs/System.md` 仅作线索。屏上箭头/线用 **`System::DrawDebug*`**，签名以编译器为准 |
...
| 照 **https://angelscript.hazelight.se/api/** 或 **Kit `AS_API/`** 写 AS 却编译失败或运行无此 API | 外站与本地归档均为 **Hazelight/通用快照**，与本项目引擎/插件/裁剪（NotInAngelscript 等）**不一致** | **能开 UE**：**as-api-dynamic-query** 核对符号 → **Script-Examples** / 编译；**勿**把线上或 `API_Index.md` 当唯一真理。离线时用 **angelscript-api-query**，并注明待 UE 内复核 |
