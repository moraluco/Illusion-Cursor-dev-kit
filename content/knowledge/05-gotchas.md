...
| 蓝图快照导出到错误目录（如 D:\\ManteumTower\\BlueprintSnapshot） | 用单参 `FPaths::ConvertRelativePathToFull(ExportDir)` 对已是绝对路径再解析，会按 CWD 重算；或依赖 `GetProjectFilePath()` 在部分环境下返回「短」路径 | 绝对路径只做 Normalize/Collapse，不调单参 ConvertRelativePathToFull；项目根用插件 `GetBaseDir()` 反推两次 GetPath，再回退 GetProjectFilePath。详见 dev/editor-tools/blueprint-export-serializer.md §8 |
